...
def model_main(request_queue, response_queue, mp_barrier):
    ...
    print(f"子进程启动。模型将缓存到简洁目录: {cache_dir}")
            ...
            if needs_reload:
                ...
                if any([settings.offload_vae, settings.offload_text_encoder, settings.offload_dit]):
                    if cpu_offload is None:
                        raise ImportError("错误: 你选择启用CPU Offload，但 `accelerate` 库未安装。请运行 `pip install accelerate`。")
                ...
                if ckpt_path.endswith('.safetensors'):
                    state_dict = load_file(ckpt_path, device="cpu" if settings.offload_dit else "cuda")
                else:
                    state_dict = torch.load(ckpt_path, map_location="cpu" if settings.offload_dit else "cuda", weights_only=False)
...
def main():
    ...
    with gr.Blocks(theme=gr.themes.Soft()) as demo:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                with gr.Accordion("生成核心参数", open=True):
                    cap = gr.Textbox(lines=3, label="正向提示 (Prompt)", value="A majestic lion overlooking the savannah at sunset, photorealistic, 8k")
                    neg_cap = gr.Textbox(lines=2, label="反向提示 (Negative Prompt)", value="blurry, low quality, cartoon, watermark")
                ...
                with gr.Accordion("采样器设置 (高级)", open=False):
                ...
                with gr.Accordion("Transport & ODE 设置 (专家)", open=False):
    ...
    mp_barrier.wait()
    print(f"Gradio UI 已就绪, 请访问: http://127.0.0.1:{args.port}")
